...
pub fn show_dashboard(app_state: &AppState, ui: &mut egui::Ui) {
    ...
    ui.horizontal(|ui| {
        ui.vertical(|ui| {
            ui.label(
                egui::RichText::new("Background Notifications")
            ...
            ui.add_space(2.0);
            ui.label(
                egui::RichText::new("Receive real-time smart alarm triggers.")
    ...
    ui.horizontal(|ui| {
        ui.vertical(|ui| {
            ...
            if is_steam_logged_in {
                ui.label(
                    egui::RichText::new("Account is currently linked.")
                        .color(egui::Color32::from_rgb(115, 115, 115))
                        .size(12.0),
                );
            } else {
                ui.label(
                    egui::RichText::new("Please link your Steam account.")
                        .color(egui::Color32::from_rgb(115, 115, 115))
                        .size(12.0),
                );
            }
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            if is_steam_logged_in {
                let btn =
                    egui::Button::new(egui::RichText::new("Logout").color(egui::Color32::BLACK))
                        ...
                        match std::process::Command::new(exe).arg("--auth").spawn() {
                            Ok(mut child) => {
                                ...
                                std::thread::spawn(move || {
                                    let _ = child.wait();
                                    if let Ok(token) =
                                        crate::config::store::Store::get_steam_token()
                                    {
                                        if let Some(tx) = &command_tx {
                                            let _ = tx.try_send(
                                                crate::ipc::IpcCommand::SetSteamToken(Some(token)),
                                            );
                                        }
                                    }
                                });
